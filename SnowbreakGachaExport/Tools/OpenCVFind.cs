...
public class OpenCVFind
{
    ...
    public static Point FindNextPageArrow()
    {
        try
        {
            var arrowMat = new Mat(nextPageArrowImagePath, ImreadModes.AnyColor).CvtColor(ColorConversionCodes.RGBA2RGB);
    ...
    public static IEnumerable<HistoryItem> FindStar(ItemType itemType)
    {
        try
        {
            ...
            var resList = list.Select(item => new HistoryItem(item, itemType, 5)).ToList();

            Console.WriteLine("4:");
            list = FindItem(fourStarMat, screenShot);
            resList.AddRange(list.Select(item => new HistoryItem(item, itemType, 4)).ToList());
            
            Console.WriteLine("3:");
            list = FindItem(threeStarMat, screenShot);
            resList.AddRange(list.Select(item => new HistoryItem(item, itemType, 3)).ToList());
    ...
    private static IEnumerable<string> FindItem(Mat starImage, Mat screenshot)
    {
        try
        {
            var list = new List<string>(); // List to return
            ...
            for (var i = 1; i < res.Rows - starImage.Rows; i += starImage.Rows)
            {
                for (var j = 1; j < res.Cols - starImage.Cols; j += starImage.Cols)
                {
                    ...
                    if (maxVal > threshold)
                    {
                        var bitmap = WindowOperate.GetCharacterNameScreenshot(j + maxLoc.X, i + maxLoc.Y, starImage.Rows);
